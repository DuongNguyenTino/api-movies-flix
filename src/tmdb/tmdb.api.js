...
const tmdbApi = {
	...
	mediaGenres: async ({ mediaType }) => await axios.get(tmdbEndpoints.mediaGenres({ mediaType })),
	...
	personDetail: async ({ personId }) => await axios.get(tmdbEndpoints.personDetail({ personId })),
	personMedias: async ({ personId }) => await axios.get(tmdbEndpoints.personMedias({ personId })),
};
